...
async def pet(session: CommandSession):
    ...
    message = message + '{0:{4}<4}:{1:>2}{4}\t{2:{4}<4}:{3:>2}\n'.format(
        pokeNameChn[chaomengInedx - 1], pokemon[chaomengInedx -
    ...
    message = message + ''.join(['[清明限定]{}:{}\n'.format(pokeNameChn[i], pokemon[i])
                                 for i in range(chaomengInedx+1, len(pokeNameChn)) if pokemon[i] > 0])
    await session.send(message[:-1])
...
async def topTicket(session: CommandSession):
    ...
    QQname = [(QQList[x[0]], x[1])
              for x in rank if x[0] in QQList and all([w not in QQList[x[0]] for w in stopWord])]
